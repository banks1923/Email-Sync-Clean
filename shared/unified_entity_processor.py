...
class UnifiedEntityProcessor:
    ...
    def process_content_entities(
        ...
        for i in range(0, len(content_records), batch_size):
            ...
            logger.info(f"Processed {processed_so_far}/{len(content_records)} content items "
        ...
        logger.info(f"✅ Entity processing complete: {stats['processed']} content items, "
    ...
    def _process_batch(self, entity_extractor, batch: list[dict]) -> dict[str, int]:
        ...
        for record in batch:
            try:
                ...
                raw_entities = extraction_result.get('entities', []) if extraction_result.get('success') else []
    ...
    def _filter_quality_entities(self, entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for entity in entities:
            ...
            if ('&' in text and len(text) < 50) or text.startswith('timing') or text.endswith('net'):
                continue
                
            # Filter OCR garbage with excessive symbols/mixed characters
            if re.search(r'[&=%£€<>©#@]{2,}', text) or re.search(r'[A-Z]{3,}[a-z]{3,}[A-Z]{3,}', text):
                continue
    ...
    def process_missing_entities_only(self, max_content: int = 100) -> dict[str, Any]:
        ...
        return self.process_content_entities(
