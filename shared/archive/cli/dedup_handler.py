...
def find_duplicates_command(
    ...
    try:
        ...
        if content_type:
            ...
            results = db.search_content("", source_type =content_type, limit=1000)
        else:
        ...
        for result in track(results, description="Processing documents..."):
            documents.append({
                ...
                'content_unified': result.get('content_unified', ''),
                'metadata': {
...
def compare_documents_command(
    ...
    try:
        ...
        similarity = detector.get_similarity(
            doc1.get('content_unified', ''),
            doc2.get('content_unified', '')
        )
        ...
        table.add_row("Type", doc1.get('source_type', 'N/A'), doc2.get('source_type', 'N/A'))
        table.add_row("Size", f"{len(doc1.get('content_unified', ''))} chars", f"{len(doc2.get('content_unified', ''))} chars")
        ...
        if show_diff and similarity > 0.3:
            ...
            console.print(doc1.get('content_unified', '')[:300] + "...")
            console.print("\n[blue]Document 2:[/blue]")
            console.print(doc2.get('content_unified', '')[:300] + "...")
...
def deduplicate_database_command(
    ...
    try:
        ...
        # Get content
        if content_type:
            results = db.search_content("", source_type =content_type, limit=5000)
        else:
        ...
        documents = [
            {
                ...
                'content_unified': r.get('content_unified', ''),
                'metadata': {
                    'title': r.get('title', ''),
        ...
        if to_delete:
            ...
            if not dry_run:
                ...
                for doc_id in track(to_delete, description="Deleting..."):
                    try:
                        # Delete FROM content_unified table
                        db.db.execute_query(
                            "DELETE FROM content_unified WHERE id = ?",
                            (doc_id,)
                        )
                        deleted += 1
                    except Exception as e:
