...
class ContentQualityScorer:
    ...
    def score_content(
        self, text: str, page_count: int = 1, entity_count: int = 0
    ) -> QualityMetrics:
        ...
        if text_length < self.MIN_TEXT_LENGTH and chars_per_page < self.MIN_CHARS_PER_PAGE:
            failure_reasons.append(
                f"insufficient_content (len={text_length}, cpp={chars_per_page:.0f})"
            )
        ...
        if digit_punct_ratio > self.MAX_DIGIT_PUNCT_RATIO:
            failure_reasons.append(
                f"high_noise_ratio ({digit_punct_ratio:.2f} > {self.MAX_DIGIT_PUNCT_RATIO})"
            )

        # Gate 4: Symbol ratio (OCR garbage)
        if symbol_ratio > self.MAX_SYMBOL_RATIO:
            failure_reasons.append(
                f"high_symbol_ratio ({symbol_ratio:.2f} > {self.MAX_SYMBOL_RATIO})"
            )

        # Gate 5: Unique bigrams (de-garble)
        if unique_bigrams < self.MIN_UNIQUE_BIGRAMS:
            failure_reasons.append(
                f"low_bigram_diversity ({unique_bigrams} < {self.MIN_UNIQUE_BIGRAMS})"
            )

        # Gate 6: English dictionary hits
        if english_dict_hit_rate < self.MIN_ENGLISH_DICT_HIT_RATE:
            failure_reasons.append(
                f"low_english_hits ({english_dict_hit_rate:.2f} < {self.MIN_ENGLISH_DICT_HIT_RATE})"
            )
        ...
        if failure_reasons:
            ...
            if entity_count > 0:
                ...
                if entities_per_kb >= self.MIN_ENTITIES_PER_KB:
                    ...
                    failure_reasons.append(
                        f"low_entity_density ({entities_per_kb:.2f} < {self.MIN_ENTITIES_PER_KB})"
                    )
            else:
                validation_status = ValidationStatus.TEXT_VALIDATED
