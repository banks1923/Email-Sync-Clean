...
class SimpleQuarantineManager:
    ...
    def quarantine_file(
        self, file_path: Path, error_msg: str, metadata: dict = None
    ) -> dict[str, Any]:
            ...
            try:
                ...
                if not quarantine_file.exists():
                    ...
                    quarantine_file = next(
                        (f for f in possible_files if not f.name.endswith(".error.json")), None
                    )
    ...
    def retry_quarantined_file(self, quarantine_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                recovered_path = (
                    recovered_dir
                    / f"{quarantine_path.stem}_recovered_{timestamp}{quarantine_path.suffix}"
                )
            ...
            return {"success": False, "error": str(e), "message": f"Exception during retry: {e}"}
    ...
    def cleanup_old_quarantine(self, days_old: int = 30) -> dict[str, Any]:
        ...
        logger.info(
            f"Quarantine cleanup: removed {len(cleaned_files)} files, freed {total_size_freed:,} bytes"
        )
