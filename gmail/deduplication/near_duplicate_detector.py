...
class LSHIndex:
    ...
    def find_similar(
        self, signature: np.ndarray, threshold: float = 0.5
...
class NearDuplicateDetector:
    ...
    def __init__(self, threshold: float = 0.8, num_perm: int = 128):
        ...
        self.lsh_index = LSHIndex(num_bands=16, band_size=num_perm // 16)
        self.processed_docs = {}
    ...
    def find_all_duplicates(self) -> dict[str, list[str]]:
        ...
        for doc_id, doc_info in self.processed_docs.items():
            ...
            similar = self.lsh_index.find_similar(doc_info["signature"], self.threshold)
    ...
    def batch_deduplicate(self, documents: list[dict]) -> dict:
        ...
        for doc in documents:
            self.add_document(
                doc.get("id", str(hash(doc["content"]))), doc["content"], doc.get("metadata", {})
            )
