...
class EnhancedOCRCoordinator:
    ...
    def process_pdf_with_enhanced_ocr(
        ...
        try:
            ...
            ocr_results = self._process_pages_with_enhanced_ocr(images, quality_gates_enabled)
            ...
            final_validation = self._perform_final_quality_validation(
                ocr_results["text"], len(images), ocr_results.get("quality_metrics")
            )
            ...
            logger.info(
                f"✅ Enhanced OCR pipeline completed in {total_time:.1f}s "
            ...
            return self._build_failure_result(
                "pipeline_exception", str(e), pipeline_metadata, processing_stages, exception=e
            )

    def _process_pages_with_enhanced_ocr(
        self, images: list, quality_gates_enabled: bool
    ) -> dict[str, Any]:
        """
        ...
        try:
            for i, image in enumerate(images):
                ...
                if page_result["success"]:
                    ...
                    logger.warning(
                        f"  ⚠ Page {i+1} OCR failed: {page_result.get('error', 'Unknown error')}"
                    )
            ...
            processing_method = (
                "mixed" if len(methods_used) > 1 else next(iter(methods_used), "unknown")
            )
            ...
            return {"success": False, "error": str(e), "pages_processed": len(page_results)}

    def _perform_final_quality_validation(
        self, text: str, page_count: int, existing_metrics: Any = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "passed_quality_gates": validation_status
                in [
    ...
    def _build_failure_result(
        ...
        failure_result = {
            ...
            - pipeline_metadata.get("started_timestamp", time.time()),
        }
    ...
    def validate_enhanced_setup(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ready": (
                base_validation.get("ready", False) and enhanced_validation.get("ready", False)
            ),
        }
