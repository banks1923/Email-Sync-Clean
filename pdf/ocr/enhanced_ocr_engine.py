...
class EnhancedOCREngine:
    ...
    def extract_text_with_dual_pass(
        self, image: Image.Image, page_count: int = 1, force_enhanced: bool = False
    ) -> dict[str, Any]:
        ...
        if not force_enhanced:
            ...
            if (
                ...
                and standard_result["confidence"]
                >= self.enhancement_config["min_confidence_for_standard"]
            ):
                ...
                if quality_metrics.validation_status == ValidationStatus.TEXT_VALIDATED:
                    processing_log.append(
                        f"✓ Standard OCR passed quality gates (confidence: {standard_result['confidence']:.2f})"
                    )
                    return {
                        "success": True,
                        "text": text,
                    ...
                    processing_log.append(
                        f"⚠ Standard OCR failed quality gates: {quality_metrics.failure_reasons}"
                    )
            else:
    ...
    def _detect_born_digital_text(self, image: Image.Image) -> dict[str, Any]:
        ...
        try:
            ...
            is_born_digital = (
                color_ratio < 0.01  # Very few unique colors
                and edge_density > 0.02  # Sufficient sharp edges
                and line_regularity > 0.7  # Regular text lines
            )
    ...
    def _enhanced_ocr_extraction(
        self, image: Image.Image, processing_log: list[str]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            kernel = cv2.getStructuringElement(
                cv2.MORPH_RECT, self.enhancement_config["morphology_kernel_size"]
            )
            ...
            ocr_data = pytesseract.image_to_data(
                enhanced_image, output_type=pytesseract.Output.DICT, config=custom_config
            )
    ...
    def validate_enhanced_setup(self) -> dict[str, Any]:
        ...
        enhanced_validation["ready"] = base_validation.get("ready", False) and CV2_AVAILABLE
