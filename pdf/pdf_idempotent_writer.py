...
class IdempotentPDFWriter:
    ...
    def write_transactional(
        self, file_path: str, chunks: list, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if existing:
            if existing["status"] == "processed":
                ...
                return {"success": True, "status": "duplicate", "sha256": sha256, "exit_code": 0}
            elif existing["status"] == "failed":
                if existing["attempt_count"] >= 3:
                    logger.warning(
                        f"PDF permanently failed after {existing['attempt_count']} attempts"
                    )
                    return {
                        "success": False,
        ...
        try:
            ...
            """,
                (
                    ...
                    ),  # First 500 chars from first 3 chunks
                    timestamp,
                ),
            )
