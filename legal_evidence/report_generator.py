...
class LegalReportGenerator:
    ...
    def generate_lookup_report(
        self, thread_ids: list[str] | None = None, keywords: list[str] | None = None
    ) -> str:
        ...
        for thread_id in threads_to_process[:20]:  # Limit to 20 threads for report size
            ...
            report.append(
                f"**Period**: {thread_summary['date_range']['start']} to {thread_summary['date_range']['end']}"
            )
            report.append("")
    ...
    def generate_narrative_report(
        self, disputed_topics: list[str], focus_senders: list[str] | None = None
    ) -> str:
        """
        ...
        if focus_senders:
            ...
            report.append(
                f"- Unique Participants: {patterns['unique_senders'] + patterns['unique_recipients']}"
            )
            ...
            if mentions:
                ...
                for mention in mentions:
                    ...
                    if any(
                        ...
                        for word in ["yes", "agree", "allow", "permitted", "free"]
                    ):
                        positions["affirmative"].append(mention)
                    elif any(
                        word in content.lower()
                        for word in ["no", "deny", "refuse", "require", "must"]
                    ):
                ...
                for position, emails in positions.items():
                    if emails:
                        report.append(
                            f"**{position.capitalize()} Statements** ({len(emails)} instances):"
                        )

                        # Show first 3 examples
                        for email in emails[:3]:
                            report.append(
                                f"- {email['eid']} ({email['datetime_utc'][:10]}, {email['sender']})"
                            )
        ...
        for thread_id in list(disputed_threads.keys())[:10]:  # Check first 10 threads
            ...
            if contradictions:
                ...
                for contradiction in contradictions:
                    report.append(
                        f"### Contradiction Found: {contradiction['sender']} on \"{contradiction['topic']}\""
                    )

                    for stmt in contradiction["statements"]:
                        report.append(
                            f"- {stmt['eid']} ({stmt['date'][:10]}): \"{stmt['statement']}\""
                        )
        ...
        if len(disputed_threads) > 0:
            report.append(
                f"1. **Pattern of Communication**: {len(disputed_threads)} separate threads"
            )
