...
class ThreadAnalyzer:
    ...
    def get_thread_summary(self, thread_id: str) -> dict[str, Any]:
        ...
        participants = set()
        subjects = []
        date_range = {"start": emails[0]["datetime_utc"], "end": emails[-1]["datetime_utc"]}
    ...
    def analyze_communication_patterns(self, sender: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "top_senders": dict(
                sorted(patterns["senders"].items(), key=lambda x: x[1], reverse=True)[:5]
            ),
        ...
        for sender, topics_dict in statements.items():
            for topic, statement_list in topics_dict.items():
                if len(statement_list) > 1:
                    ...
                    has_positive = any(
                        "yes" in s["statement"].lower()
                        or "agree" in s["statement"].lower()
                        or "allow" in s["statement"].lower()
                        or "free" in s["statement"].lower()
                        for s in statement_list
                    )
                    has_negative = any(
                        "no" in s["statement"].lower()
                        or "disagree" in s["statement"].lower()
                        or "deny" in s["statement"].lower()
                        or "require" in s["statement"].lower()
                        for s in statement_list
                    )

                    if has_positive and has_negative:
                        contradictions.append(
                            {"sender": sender, "topic": topic, "statements": statement_list}
                        )
