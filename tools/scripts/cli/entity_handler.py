...
class EntityHandler:
    ...
    def extract_entities_unified(self, args) -> dict[str, Any]:
        ...
        try:
            if missing_only:
                ...
                result = self.processor.process_missing_entities_only(max_content=max_content or 100)
            else:
    ...
    def entity_status(self, args) -> dict[str, Any]:
        ...
        try:
            ...
            overall_coverage = (total_with_entities / total_content * 100) if total_content > 0 else 0
            print(f"\n📈 Overall:")
            ...
            if status['entity_type_distribution']:
                ...
                sorted_types = sorted(
                    status['entity_type_distribution'].items(), 
                    key=lambda x: x[1], 
                    reverse=True
                )[:10]
            ...
            if missing_entities > 0:
                ...
                print(f"  Run: tools/scripts/vsearch extract-entities --missing-only")
    ...
    def search_by_entity(self, args):
        ...
        try:
            ...
            results = self.processor.db.fetch(
                query, 
                (entity_type.upper(), f'%{entity_value}%', limit)
            )
