        ...
        test_points = [
            {
                "id": str(uuid.uuid4()),
                "vector": emb,
                "metadata": {"type": "diagnostic", "index": i}
            }
...
def check_maintenance() -> tuple[bool, dict]:
    ...
    try:
        ...
        has_wal_checkpoint = hasattr(db, 'db_maintenance') and callable(getattr(db, 'db_maintenance'))
...
def run_diagnostics() -> int:
    ...
    if align_ok:
        print(f"ALIGNMENT: dim_embed={align_data['embed_dim']} dim_collection={align_data['collection_dim']} "
              f"dim_match={align_data['dim_match']} l2_norm={align_data['l2_norm']}  {format_status(align_ok)}")
    else:
        print(f"ALIGNMENT: FAILED - {align_data.get('error', 'Unknown error')}  {format_status(align_ok)}")
    ...
    if embed_ok:
        print(f"EMBEDDINGS: device={embed_data['device']} batch={embed_data['batch_size']} "
    ...
    if vector_ok:
        print(f"VECTORS: batch_upsert={vector_data['batch_upsert']} iter_ids={vector_data['iter_ids']} "
              f"upsert={vector_data['upsert_per_sec']}/s  {format_status(vector_ok)}")
    else:
        print(f"VECTORS: FAILED - {vector_data.get('error', 'Unknown error')}  {format_status(vector_ok)}")
    ...
    if search_ok:
        print(f"SEARCH: k=10 q={search_data['queries']} p95={search_data['p95_ms']} ms "
              f"top1={search_data['best_score']}  {format_status(search_ok)}")
    else:
        print(f"SEARCH: FAILED - {search_data.get('error', 'Unknown error')}  {format_status(search_ok)}")
    ...
    if maint_ok:
        print(f"MAINTENANCE: batch={maint_data['batch_size']} progress={maint_data['progress_every']} "
              f"wal_checkpoint={maint_data.get('checkpoint_ms', 0)} ms  {format_status(maint_ok)}")
    else:
        print(f"MAINTENANCE: FAILED - {maint_data.get('error', 'Unknown error')}  {format_status(maint_ok)}")
    ...
    if "error" not in reconcile_data:
        if "scan_rate" in reconcile_data and reconcile_data["scan_rate"] == "N/A - iter_ids not available":
