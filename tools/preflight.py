...
class PreflightChecker:
    ...
    def check_database_schema(self):
        ...
        try:
            ...
            # Check indexes
            cursor = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name LIKE 'ix_content_%'"
            )
        ...
        try:
            ...
            if all_present:
                try:
                    ...
                    self._print_check(
                        "get_all_content_ids() works", True, f"Found {len(ids)} content IDs"
                    )

                    if len(ids) >= 1:
                        content = db.get_content_by_ids(ids[:1])
                        self._print_check(
                            "get_content_by_ids() works", True, f"Retrieved {len(content)} items"
                        )
    ...
    def check_cli_tools(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python3", "tools/scripts/vsearch", "--help"],
                capture_output=True,
                text=True,
                timeout=3,
            )
    ...
    def check_vector_parity(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python3", "tools/preflight/vector_parity_check.py"],
                capture_output=True,
                text=True,
                timeout=30,
                env=env,
            ...
            if result.returncode in [0, 1]:  # 0 = OK, 1 = Warning
                ...
                try:
                    ...
                    self._print_check(
                        "Collection exists", qdrant_info.get("collection_exists", False)
                    )
                    self._print_check(
                        "Vector dimensions",
                        qdrant_info.get("dimension") == 1024,
