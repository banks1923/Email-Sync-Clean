...
def format_service_table(services: list) -> str:
    ...
    sorted_services = sorted(
        existing_services,
        key=lambda s: s.get("total_lines", 0) if isinstance(s, dict) else s.total_lines,
        reverse=True,
    )
    ...
    total_lines = sum(
        s.get("total_lines", 0) if isinstance(s, dict) else s.total_lines for s in existing_services
    )
    total_code = sum(
        s.get("code_lines", 0) if isinstance(s, dict) else s.code_lines for s in existing_services
    )
...
def update_readme_md(audit_report: dict, file_path: Path) -> bool:
    ...
    patterns = [
        (
            r"550 lines replacing 2000\+ lines \(75% reduction\)",
            f"{total_lines:,} lines across all services",
        ),
