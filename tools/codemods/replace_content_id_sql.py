...
SQL_PATTERNS = [
    ...
    (
        re.compile(
            r"UPDATE\s+content\s+SET\s+(.*?)\s+WHERE\s+content_id\s*=\s*\?",
    ...
    (re.compile(r"FOREIGN\s+KEY\s*\(\s*content_id\s*\)", re.IGNORECASE), "FOREIGN KEY (id)"),
...
SQL_MARKERS = re.compile(
    r"\b(SELECT|INSERT|UPDATE|DELETE|CREATE\s+TABLE|ALTER\s+TABLE|DROP\s+TABLE|REFERENCES|FOREIGN\s+KEY|PRIMARY\s+KEY)\b",
    re.IGNORECASE,
)
...
Examples:
    ...
    group.add_argument("--apply", action="store_true", help="Apply the changes to files")

    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show verbose output including unchanged files"
    )
