...
def report_command(
    output_dir: str = "legal_evidence_export",
...
def status_command():
    ...
    if stats["emails_without_eid"] > 0:
        print(
            f"  - Run 'vsearch evidence assign-eids' to assign {stats['emails_without_eid']} missing EIDs"
        )
    ...
    if stats["emails_with_eid"] > 0:
        print(
            "  - Run 'vsearch evidence report --keywords \"entry,access\"' to generate evidence reports"
        )
