    ...
    class MCPConfig(BaseSettings):
            ...
            servers = {
                ...
                "sequential-thinking": {
                    ...
                    "env": {
                        ...
                        "MCP_STORAGE_DIR": str(
                            self.project_root / "data/system_data/sequential_thinking"
                        ),
                    },
                },
            }
        ...
        def validate_config(self) -> dict[str, str]:
            ...
            status["legal_mcp"] = (
                "✅ Found"
                if (
                    self.project_root / "infrastructure/mcp_servers/legal_intelligence_mcp.py"
                ).exists()
                else "❌ Missing"
            )
            status["search_mcp"] = (
                "✅ Found"
                if (
                    self.project_root / "infrastructure/mcp_servers/search_intelligence_mcp.py"
                ).exists()
                else "❌ Missing"
            )
            ...
            status["sequential_thinking"] = (
                "✅ Found" if seq_thinking_path.exists() else "⚠️ Missing"
            )
            ...
            status["api_keys"] = (
                f"✅ {api_count} configured" if api_count > 0 else "⚠️ None configured"
            )
