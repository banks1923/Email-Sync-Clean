...
class TestSchemaInvariants(unittest.TestCase):
    ...
    def test_sql_no_content_id_strings(self):
        ...
        for py_file in project_root.rglob('*.py'):
            ...
            if any(skip_path in str(py_file) for skip_path in [
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if any(keyword in line.upper() for keyword in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'REFERENCES']):
    ...
    def test_upsert_idempotent_business_key(self):
        ...
        try:
            db.execute("DELETE FROM content WHERE source_type='test-invariant' AND external_id LIKE 'test-%'")
        except:
        ...
        try:
            ...
            records = db.fetch("SELECT COUNT(*) as count FROM content WHERE source_type='test-invariant' AND external_id='test-001'")
            self.assertEqual(1, records[0]["count"], "Should have exactly one record with this business key")
    ...
    def test_qdrant_id_consistency(self):
        ...
        try:
            ...
            self.assertEqual(content_id, result.get("id"), "Vector point ID should match content ID")
    ...
    def test_business_key_constraint_enforced(self):
        ...
        # Clean up any existing test data first
        try:
            db.execute("DELETE FROM content WHERE source_type='test-constraint' AND external_id='unique-test-001'")
        except:
            pass
        ...
        try:
            ...
            with self.assertRaises(Exception):
                db.execute("""
                    INSERT INTO content (id, source_type, external_id, content_type, title, content)
                    VALUES ('different-uuid', 'test-constraint', 'unique-test-001', 'test', 'Duplicate', 'Duplicate content')
                """)
    ...
    def test_deterministic_uuid_namespace(self):
        ...
        # Clean up any existing test data first
        try:
            db.execute("DELETE FROM content WHERE source_type='test-namespace' AND external_id='namespace-test-001'")
        except:
            pass
        ...
        try:
            ...
            content_id = db.upsert_content(
                ...
                external_id="namespace-test-001",
                content_type="test",
            ...
            self.assertEqual(expected_id, content_id, 
