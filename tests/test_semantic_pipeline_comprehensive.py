...
class TestSemanticPipeline(unittest.TestCase):
    ...
    def _insert_test_data(self):
        ...
        for email in test_emails:
            self.db.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (email['message_id'], email['subject'], email['sender'], 
                 email['content'], email['datetime_utc'], email['eid'], email['thread_id']))
            ...
            self.db.execute("""
                INSERT INTO content (content_type, content, metadata)
                VALUES (?, ?, ?)
            """, ('email', email['content'], json.dumps(content_metadata)))
    ...
    def test_idempotency(self):
        ...
        result2 = self.pipeline.run_for_messages(
            message_ids=message_ids,
            steps=['entities']
        )
    ...
    def test_batch_processing(self):
        ...
        for i in range(10):
            self.db.execute("""
                INSERT INTO emails (message_id, subject, sender, content, datetime_utc, eid, thread_id)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (f'msg_batch_{i}', f'Subject {i}', 'test@example.com',
                 f'Content {i}', '2025-01-13T10:00:00Z', f'EID-2025-{i:04d}', f'thread_{i}'))
