...
class TestSchemaMigration:
    ...
    def test_business_key_uniqueness(self, temp_db_path):
        ...
        if hasattr(db, 'upsert_content'):
            ...
            assert content_id_1 == content_id_2, "UPSERT should return same ID for same business key"
            
            # Verify only one record exists
            records = db.fetch("SELECT COUNT(*) as count FROM content WHERE source_type = 'email' AND external_id = 'test123'")
