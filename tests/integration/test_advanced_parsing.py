...
def test_advanced_parsing():
    ...
    for email in sample_emails:
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                print(
                    f"   📄 Message {i+1}: {len(msg.content)} chars, sender: {msg.sender or 'Unknown'}"
                )
    ...
    try:
        ...
        if total_messages_extracted > 0:
            ...
            print(
                f"   📝 Would store message: {len(test_message.content)} chars from {test_message.sender}"
            )
...
def show_current_stats():
    ...
    message_stats = db.fetch_one(
        "SELECT COUNT(*) as count FROM content_unified WHERE source_type = 'email_message'"
    )
    print(f"💬 Individual messages: {message_stats['count']}")

    # Thread stats
    thread_stats = db.fetch_one(
        "SELECT COUNT(DISTINCT thread_id) as count FROM emails WHERE thread_id IS NOT NULL"
    )
