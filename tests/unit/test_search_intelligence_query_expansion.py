...
class TestQueryExpansion:
    ...
    def service(self):
        ...
        """
        with patch("search_intelligence.main.SimpleDB"), patch(
            "search_intelligence.main.get_vector_store"
        ), patch("search_intelligence.main.EntityService"), patch(
            "search_intelligence.main.get_embedding_service"
        ), patch(
            "search_intelligence.main.get_document_summarizer"
        ):

            service = SearchIntelligenceService()
            # Mock the database search to return test data
            service.db.search_content = Mock(
                return_value=[
                    {
                        "id": "1",
                        "title": "Test Doc",
    ...
    def test_smart_search_with_expansion_creates_or_query(self, mock_logger, service):
        ...
        service.db.fetch = Mock(
            return_value=[{"id": "1", "title": "Test", "body": "content", "source_type": "email"}]
        )
    ...
    def test_filter_support_with_expansion(self, service):
        ...
        result = service.smart_search_with_preprocessing(
            "legal", filters=filters, use_expansion=True
        )
...
class TestParameterValidation:
    ...
    def test_extract_and_cache_entities_parameter_validation(self, service):
        ...
        """
        # Mock dependencies
        service.db.get_content = Mock(return_value={"body": "test content", "title": "test"})
        service.entity_service.extract_email_entities = Mock(
            return_value={"entities": [{"text": "test", "label": "TEST"}]}
        )
    ...
    def test_analyze_document_similarity_parameter_validation(self, service):
        ...
        service.db.get_content = Mock(return_value={"content": "test document content"})
