...
def bench_write(db: SimpleDB, num_docs: int = 10000) -> dict:
    ...
    for batch_start in range(0, num_docs, 100):
        ...
        for data, hash_val in batch_data:
            db.execute(
                "INSERT OR IGNORE INTO bench_test (data, hash) VALUES (?, ?)", (data, hash_val)
            )
    ...
    return {
        ...
        "p95_ms": (
            round(statistics.quantiles(times, n=20)[18], 2) if len(times) > 20 else max(times)
        ),
...
def run_benchmark():
    ...
    """
    results = {"timestamp": datetime.now().isoformat(), "baseline": {}, "optimized": {}}
