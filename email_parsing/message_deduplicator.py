...
class MessageDeduplicator:
    ...
    def parse_email_thread(self, email_content: str, email_id: str, 
                          thread_id: str | None = None) -> list[ParsedMessage]:
        ...
        """
        logger.debug(f"Parsing email {email_id}: {len(email_content)} chars, {email_content.count(chr(10))} lines")
        messages = []
        ...
        for i, boundary in enumerate(boundaries):
            logger.debug(f"Email {email_id}: Processing boundary {i+1}: {boundary.boundary_type} lines {boundary.start}-{boundary.end}")
            ...
            message = self._parse_message_segment(
                content_slice,
            ...
            if message:
                if self._is_substantial_content(message.content):
                    ...
                    logger.debug(f"Email {email_id}: ✅ Added message {i+1}: {message.context_type}, {len(message.content)} chars")
                else:
                    logger.debug(f"Email {email_id}: ❌ Filtered message {i+1}: Not substantial ({len(message.content)} chars)")
            else:
        ...
        logger.info(f"Email {email_id}: Extracted {len(messages)} messages from {len(boundaries)} boundaries")
        return messages
        ...
        for i, line in enumerate(lines):
            for pattern in self.FORWARD_MARKERS:
                if re.match(pattern, line, re.IGNORECASE):
                    ...
                    boundaries.append(MessageBoundary(
                        start=i,
                        end=end,
    ...
    def _merge_boundaries(self, boundaries: list[MessageBoundary], 
                          total_lines: int) -> list[MessageBoundary]:
        """
        ...
        for boundary in boundaries:
            ...
            if boundary.start > current_pos:
                final_boundaries.append(MessageBoundary(
                    start=current_pos,
                    end=boundary.start,
                    boundary_type='original',
                    confidence=1.0,
    ...
    def _parse_message_segment(self, content: str, boundary_type: str, 
                               position: int) -> ParsedMessage | None:
        """
    ...
    def _extract_date(self, content: str) -> datetime | None:
        ...
        if match:
            ...
            for fmt in ['%a, %d %b %Y %H:%M:%S', '%Y-%m-%d %H:%M:%S', '%m/%d/%Y %H:%M',
                       'Mon, %d Jan %Y %H:%M:%S', '%a, %d %b %Y %H:%M:%S']:
                try:
...
def process_email_file(email_content: str, email_id: str) -> tuple[list[dict], dict]:
    ...
    logger.info(f"Processed email {email_id}: {stats['total_messages']} messages, "
