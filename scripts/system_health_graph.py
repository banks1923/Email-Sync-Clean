#!/usr/bin/env python3
"""
System Health Check Graph Generator Generates comprehensive health
visualizations for the Email Sync System.
"""
...
def get_file_system_stats():
    ...
    for name, path in directories.items():
        if path.exists():
            ...
            stats[f"{name}_size_mb"] = sum(f.stat().st_size for f in files if f.is_file()) / (
                1024 * 1024
            )
        else:
...
def create_system_health_dashboard():
    ...
    statuses = [
        service_status.get("qdrant", "offline"),
    ...
    for i, (bar, status) in enumerate(zip(bars1, statuses)):
        ax1.text(0.5, i, status.upper(), ha="center", va="center", fontweight="bold", color="white")
    ...
    non_zero_sizes = [
        (cat, size) for cat, size in zip(storage_categories, storage_sizes) if size > 0
    ]
    if non_zero_sizes:
    ...
    if db_stats:
        total_records = sum(
            db_stats.get(table, 0) for table in ["content_unified", "documents", "embeddings"]
        )
    ...
    # File system summary
    total_files = sum(
        fs_stats.get(f"{cat}_files", 0) for cat in ["user_data", "system_data", "backups"]
    )
    total_storage = sum(
        fs_stats.get(f"{cat}_size_mb", 0) for cat in ["user_data", "system_data", "backups"]
    )
    ...
    ax6.text(
        0.05,
        0.05,
        f"Overall Status: {overall_health.upper()}",
        transform=ax6.transAxes,
        fontsize=14,
        fontweight="bold",
        color=health_color,
...
def main():
    ...
    output_file = (
        output_dir / f"system_health_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    )
