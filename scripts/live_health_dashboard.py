...
Real-time updating dashboard with web interface
"""
...
class LiveHealthDashboard:
    def __init__(self):
        ...
        self.ani = animation.FuncAnimation(
            self.fig, self.update_plots, interval=5000, blit=False
        )
    ...
    def create_control_panel(self):
        ...
        ttk.Button(control_frame, text="Force Refresh", 
                  command=self.force_refresh).pack(side=tk.RIGHT, padx=5)
        ...
        interval_spinbox = ttk.Spinbox(control_frame, from_=1, to=60, width=5,
        ...
        while self.running:
            try:
                if self.auto_refresh.get():
                    ...
                    # Update status
                    healthy_services = sum(1 for status in new_data['services'].values() 
                                         if status == 'healthy')
    ...
    def update_plots(self, frame):
        ...
        if 'database' in data and isinstance(data['database'], dict):
            db_data = {k: v for k, v in data['database'].items() 
                      if k != 'size_mb' and isinstance(v, int)}
            if db_data:
        ...
        if len(self.data_history) > 1:
            ...
            if 'content_unified' in data.get('database', {}):
                content_counts = [d.get('database', {}).get('content_unified', 0) 
        ...
        if 'database' in data and isinstance(data['database'], dict):
            total_records = sum(v for k, v in data['database'].items() 
                              if isinstance(v, int) and k != 'size_mb')
