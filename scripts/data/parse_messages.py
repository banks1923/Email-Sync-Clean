...
class EmailBatchProcessor:
    ...
    def process_email(self, email_path: Path) -> tuple[int, int]:
        ...
        try:
            ...
            for msg_hash, msg_data in unique_messages.items():
                ...
                if is_new:
                    ...
                    self.db.add_content(
                        ...
                        title=msg_data["subject"]
                        or f"Message from {msg_data['sender'] or 'Unknown'}",
                        content=msg_data["content"],
            ...
            logger.info(
                f"Processed {email_path.name}: {len(messages)} messages, "
            ...
            self.stats["errors"].append({"file": str(email_path), "error": str(e)})
            raise
    ...
    def process_all(self, resume: bool = True) -> dict:
        ...
        for i, email_path in enumerate(email_files, 1):
            ...
            try:
                ...
                if i % 10 == 0:
                    ...
                    logger.info(
                        f"Progress: {i}/{len(email_files)} files, " f"{rate:.1f} files/second"
                    )
        ...
        total_messages = self.db.fetch_one("SELECT COUNT(*) as count FROM individual_messages")
...
def main():
    ...
    parser.add_argument(
        "--db-path", help="Path to SQLite database", default="data/system_data/emails.db"
    )
    parser.add_argument(
        "--no-resume", action="store_true", help="Process all files, ignore resume state"
    )
    parser.add_argument(
        "--reset", action="store_true", help="Reset processing state and start fresh"
    )
