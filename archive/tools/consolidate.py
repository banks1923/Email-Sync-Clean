...
def rewrite_imports(py_files, import_rewrites, dry_run=True):
    changed = 0
    rules = [(r['from'], r['to']) for r in import_rewrites]
...
def delete_globs(patterns, dry_run=True):
    ...
    removed = []
    for pat in patterns:
        abs_pat = str(ROOT / pat)
        for path_str in _glob.glob(abs_pat, recursive=True):
            p = Path(path_str)
            if dry_run:
                removed.append(p)
            else:
...
def main():
    ...
    # Initial scan (pre-move)
    pre_files = list_py_files()
    print(f'Python files scanned (pre-move): {len(pre_files)}')

    # Moves (and dir creations)
    ...
    # Rescan after moves so we don't reference stale paths
