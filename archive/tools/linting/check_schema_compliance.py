        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if not ('"' in line or "'" in line):
                    continue
                if not re.search(
                    r"\b(SELECT|WHERE|INSERT|UPDATE|DELETE|CREATE|REFERENCES)\b",
                    line,
                    re.IGNORECASE,
                ):
                    continue
                ...
                sql_parts = re.findall(
                    r'["\']([^"\']*(?:SELECT|WHERE|INSERT|UPDATE|DELETE|CREATE|REFERENCES)[^"\']*)["\']',
                    line,
                    re.IGNORECASE,
                )
...
def main():
    ...
    if content_id_violations:
        ...
        print(
            "  💡 Use 'id' instead of 'content_id' in SQL, or add '# ALLOWED: content_id' comment"
        )
        all_passed = False
    else:
