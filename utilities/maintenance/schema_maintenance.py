    ...
    def fix_content_schema(self, dry_run: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT id, content_type, external_id, text, metadata
                    FROM unified_content
                """)
                rows = cursor.fetchall()
                
                for row in rows:
                    content_id, content_type, external_id, text, metadata = row
                    issues = []
                    ...
                    if issues:
                        ...
                        if not dry_run:
                            ...
                            if "missing_content_type" in issues:
                                updates.append("content_type = ?")
                                params.append("unknown")
                                
                            if "missing_external_id" in issues:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    FROM emails e
                    LEFT JOIN unified_content uc ON uc.external_id = e.id
                    WHERE uc.id IS NULL
                    LIMIT ?
                """, (batch_size,))
                ...
                for email in emails:
                    ...
                    try:
                        ...
                        cursor.execute("""
                            INSERT INTO unified_content (
                                content_type, external_id, text, metadata, created_at
                            ) VALUES (?, ?, ?, ?, ?)
                        """, (
                            'email',
                            email_id,
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE content_type IS NULL OR external_id IS NULL
                """)
                ...
                if duplicates:
                    validation_results["issues"].append(
                        f"Found {len(duplicates)} duplicate external_ids"
                    )
...
def main():
    ...
    if result.get('status') not in ['completed', 'healthy', 'no_legacy_table', 'all_migrated', 'nothing_to_migrate']:
        sys.exit(1)
