...
class SemanticWiringVerifier:
    ...
    def spot_check_linkage(self) -> bool:
        ...
        try:
            ...
            point_id = hashlib.md5(message_id.encode()).hexdigest()[:8] + "-" + \
    ...
    def check_idempotency(self) -> bool:
        ...
        print(f"{'✅' if idempotent else '⚠️ '} Idempotency: {'PASS' if idempotent else 'WARN - duplicates created'}")
    ...
    def check_failure_signals(self) -> bool:
        ...
        try:
            ...
            if len(vector_results) > 0 and len(hybrid_results) == 0:
                issues.append("❌ Hybrid search returns 0 while vector returns >0 - payload/join issue")
            else:
                print(f"✅ Search consistency: vector={len(vector_results)}, hybrid={len(hybrid_results)}")
        ...
        if total > 0:
            ...
            if entity_coverage < 10 and total > 50:
                issues.append(f"⚠️  Low entity coverage: {entity_coverage:.1f}% - TTL or extraction issue")
            else:
        ...
        if dup_timeline:
            issues.append(f"⚠️  Possible timeline over-deduplication: {dup_timeline[0]} events for same content/type/date")
        else:
