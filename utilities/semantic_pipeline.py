...
class SemanticPipeline:
    ...
    def _get_email_data(self, message_ids: List[str]) -> List[Dict[str, Any]]:
        """Get email data from v2.0 schema (individual_messages + content_unified)."""
        if not message_ids:
            return []
            ...
            try:
                ...
                for email in batch:
                    if email.get('content_id'):
                        ...
                        cursor = self.db.execute("""
                            SELECT metadata FROM content_unified WHERE id = ?
                        """, (email['content_id'],))
                        ...
                        if row:
                            ...
                            self.db.execute("""
                                UPDATE content_unified SET metadata = ? WHERE id = ?
